...
(function ($) {
    ...
    var WIDTH = 1280, HEIGHT = 720,
         ...
         */
		VIDEO_SLUG = '<video width="' + WIDTH + '" height="' + HEIGHT + '" src="movies/%id.%ext" data-autoplay="%autoplay"></video>',
    ...
    $(function () {
		...
		$.each(videoSlides, function (idx, vset) {
			...
			$("[data-index=" + idx + "]").append(VIDEO_SLUG.replace("%id", video).replace("%autoplay", autoplay ? "yes" : "no"));
		});
		...
		$("[id^=drawer]").on("click", function (event) {
			event.preventDefault();
			var e = $(event.target),
				s = e.parents(".screen");
			if (e.attr("id") === "drawer-geochemistry") {
				var ee = s.find("video").show(),
					me = ee[0];
					me.play();
					setTimeout(function () {
						me.currentTime = 0;
