...
(function ($) {
    ...
    var WIDTH = 1280, HEIGHT = 720,
         ...
         */
		VIDEO_SLUG = '<video width="' + WIDTH + '" height="' + HEIGHT + '" src="movies/%id.%ext"></video>',
	...
	VIDEO_SLUG = VIDEO_SLUG.replace("%ext", (window.mozInnerScreenX === null) ? "ogv" : "mov");
     ...
     * the labyrinth.
     */
    function cleanupScroll(bakwards) {
    ...
    $(function () {
        ...
        $(".mover").on("click", function (event) {
            ...
            $("#viewport-scroller").animate(o, SLIDE_TIME, function () {
                cleanupScroll(backward);
            });
        });
		
		$.each(videoSlides, function (idx, video) {
			$("[data-index=" + idx + "]").html(VIDEO_SLUG.replace("%id", video));
		});
		...
		$("[data-index]")
			...
			.on("activate", function (event) {
				...
				if (videoSlides[e.attr("data-index")]) {
					...
					// Once a video has been played remove it
					delete videoSlides[e.attr("data-index")];
				}
			});
